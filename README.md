...
Milestone 3 extends WebScanPro by introducing **authentication and session security analysis**.

This module focuses on identifying **weak authentication mechanisms and session handling issues** that attackers commonly exploit.
...
✔ Authentication Vulnerability Reporting  
✔ JSON-Based Security Findings  
...
The module tests whether the application allows login using **common or weak credentials**, which is a common vulnerability in poorly secured systems.

It also examines **session cookies** generated by the application to understand how user sessions are maintained.
...
The response is analyzed to determine whether login was successful.

If the response does not contain the **"Login failed"** message, the system flags it as a **weak authentication vulnerability**.
...
These cookies are used by the application to manage authenticated sessions.
...
![Auth JSON](Week-5/screenshots/auth_results_output.png)  
...
✔ Session cookie discovery integrated  
...
Milestone 3 enhances WebScanPro with **authentication security analysis capabilities**.
